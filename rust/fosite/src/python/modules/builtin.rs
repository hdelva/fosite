...
pub fn new_builtin_module(vm: &mut VirtualMachine) -> Module {
    let mut builtin = Module::new();

    define_int_cast(&mut builtin, vm);
    define_float_cast(&mut builtin, vm);
    
    define_input(&mut builtin, vm);
    define_print(&mut builtin, vm);

    define_abs(&mut builtin, vm);
    define_round(&mut builtin, vm);
    define_ord(&mut builtin, vm);

    define_range(&mut builtin, vm);
    
    builtin
}

fn define_int_cast(module: &mut Module, vm: &mut VirtualMachine) {
...
fn define_float_cast(module: &mut Module, vm: &mut VirtualMachine) {
    let outer = |vm: &mut VirtualMachine| {
...
fn define_print(module: &mut Module, vm: &mut VirtualMachine) {
    let outer = |vm: &mut VirtualMachine| {
        let pointer = vm.object_of_type(&"function".to_owned());
...
fn define_abs(module: &mut Module, vm: &mut VirtualMachine) {
    let outer = |vm: &mut VirtualMachine| {
        let pointer = vm.object_of_type(&"function".to_owned());
...
fn define_round(module: &mut Module, vm: &mut VirtualMachine) {
    let outer = |vm: &mut VirtualMachine| {
        let pointer = vm.object_of_type(&"function".to_owned());
...
fn define_ord(module: &mut Module, vm: &mut VirtualMachine) {
    let outer = |vm: &mut VirtualMachine| {
        let pointer = vm.object_of_type(&"function".to_owned());
...
fn define_range(module: &mut Module, vm: &mut VirtualMachine) {
    let outer = |vm: &mut VirtualMachine| {
        let pointer = vm.object_of_type(&"function".to_owned());
        ...
        let inner = | env: Environment, args: Vec<Mapping>, _: &HashMap<String, GastNode> | {
            ...
            let list_type = vm.knowledge().get_type(&list_type_name).unwrap().clone();
